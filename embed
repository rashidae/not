...
(()=> {
  ...
  */
        ...
        const spinAnimationStyles=` @keyframes spin {
            0% {
        ...
        `;
        const loadingStyles=` position: absolute;
        top: 50%;
        left: 50%;
        ...
        const getMobilePopupStyles=(embedType)=> ` @media(max-width: 480px) {
            .fillout-embed-$ {
                embedType
            }
    ...
    don't want to overlay it on top of logos or back buttons) */
            ...
            .fillout-embed-iframe-container iframe {
                border-radius: 0;
            }
        ...
        `;
        const popupBackgroundStyles=` position: fixed;
        top: 0;
        left: 0;
        width: 100%;
        height: 100%;
   ...
   * iframe is.*/
        ...
        `;
        const XIcon=` <svg xmlns="http://www.w3.org/2000/svg"fill="none"viewBox="0 0 24 24"stroke="currentColor"stroke-width="2"> <path stroke-linecap="round"stroke-linejoin="round"d="M6 18L18 6M6 6l12 12"/> </svg> `;
        ...
        const getConfig=(element)=> {
            ...
            const hexToRgb=(hex)=> {
                ...
                if (typeof hex !=='string'|| !/^#([A-Fa-f0-9] {
                            6
                        }
        ...
        const getSharedIframeSrc=(configDomain, flowPublicIdentifier, inheritParameters, target)=> {
            ...
            for (const attribute of target.attributes) {
                if (attribute.name.startsWith('data-') && !attribute.name.startsWith('data-fillout')) {
        ...
        const initializePopupButton=(element, onclick)=> {
            ...
            const {
                buttonText, buttonColor, buttonTextColor, buttonFloat, buttonSize,
            }
            ...
            Object.assign(button.style, Object.assign(Object.assign( {
            ...
            const buttonClassName=`fillout-embed-popup-button-$ {
                buttonId
            }
        ...
        const initializePopupLikeTarget=(target, embedType)=> {
            const {
                flowPublicIdentifier, initialized, inheritParameters, sliderDirection, domain,
            }
            ...
            popupContainer.className=`fillout-embed-$ {
                embedType
            }
            ...
            closeIcon.className=`fillout-embed-$ {
                embedType
            }
        ...
        const initializeStandardTarget=(target, isFullScreen)=> {
            const {
                initialized, flowPublicIdentifier, inheritParameters, dynamicResize, domain,
            }
            ...
            iframeSrc.searchParams.append('fillout-embed-id', `$ {
                    embedId
                }
            ...
            if (dynamicResize) {
                ...
                const receiveMessage=(event)=> {
                    if (event.origin !==new URL(iframeSrc.toString()).origin || event.data.embedId !==embedId) {
